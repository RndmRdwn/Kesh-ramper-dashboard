  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl">
        ...
        <DialogFooter >
          <Button onClick={() => setIsDeleting(true)} size="sm" variant={'destructive'} className="px-4">
            <Trash2 /> Delete
          </Button>
          <Button onClick={() => setIsEditable(!isEditable)} size="sm" variant={isEditable ? 'outline' : 'default'} className="px-4"><Pencil /> 
            {!isEditable ? 'Edit' : 'Save'}
          </Button>
        </DialogFooter>
      </DialogContent>
      <DeleteMember data={data} onClose={() => setIsDeleting(!isDeleting)} open={isDeleting} />
    </Dialog>
  );
};
